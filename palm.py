...
palm.configure(api_key="AIzaSyC46bA1dRHL3VLP23_JGH0EyYg3QpyeNOA")
...
"""
user_k = """\nQ:give the graph schema structure result only in GSQL format properly with the correct syntax as given above.
For Edge Creation Use "UNDIRETCTED EDGE" only .Dont Give None as output i want GSQL queries only.
break the column headers into several vertices and make inter connection."""
def get_answer(user_prompt):
    palm.configure(api_key=user_prompt['key'])
    model_id="models/text-bison-001"
    prompt = f""" 
    {sys_prompt} \n 
    {context}\n 
    Topic:{user_prompt['title']}\n
    Dataset:\n
    {user_prompt['datasets']}\n 
    ``` 
    {user_prompt["business_req"]}
    """
    ...
    print("not transformed ",gsql_query)
